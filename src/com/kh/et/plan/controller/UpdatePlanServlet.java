...
public class UpdatePlanServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		reqPlan.setpNo(Integer.parseInt(planNo));
		...
		int result = new PlanService().updatePlan(reqPlan, planDetailList);
		
		String page = "";
		if(result > 0) {
			page = "selectPlanList.pl?mno="+Integer.parseInt(pWriter);
			request.setAttribute("msg", "플랜 수정이 잘되었습니다.");
		} else {
			page = "selectPlanList.pl?mno="+Integer.parseInt(pWriter);
