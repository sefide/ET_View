 ...
 */
@WebServlet("/paymentPremiumCouponCompany.pco")
public class CompanyPremiumCouponSevlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CompanyPremiumCouponSevlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = new PaymentService().insertPremiumPayment(p,c);
		
		String page="";
		if(result>0) {
			int count = p.getPrice()/2;
			Company loginUser = (Company)request.getSession().getAttribute("loginCompany");
			int preC = loginUser.getcPremium()+count;
			loginUser.setcPremium(preC);
